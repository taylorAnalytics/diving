<?php
/* This script will add a new dive to the trip. It will:
 * - show the form to collect dive data
 * - handle the form
 * - create the new object of class Dive
 * - use the create() method of the object to save dive data into MySQL database
 */
// Include the lists for the select fields in the form
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	if (isset($_POST['shopId'])) {
		if ($_POST['shopId']==0) {
			...
			$stmt=$pdo->prepare($q);
			$r=$stmt->execute(array(':shopId'=>$_POST['shopId']));
			if ($r) {
	...
	if (!$problem) {
		// Create new object of the class Dive
		$dive = new Dive();
		if ($dive->create()) {
			// Update the number of dives per dive shop in the database
			// Count the number of dives within the particular diveshop and insert it into shopstrips database (with a single query)
			$q='UPDATE shopstrips SET noOfDives = (SELECT COUNT(diveId) FROM dives WHERE (shopId=:shopId) AND (tripId=:tripId)) WHERE (shopId=:shopId) AND (tripId=:tripId)';
			$stmt=$pdo->prepare($q);
			$r=$stmt->execute(array(':shopId'=>$_POST['diveshop'], ':tripId'=>$_GET['tripId']));
			if ($r) {// The query has been run
				echo '<p class="error">Your dive has been created</p>';
			} else {
				echo '<p class="error">Something went wrong. Could not create your dive in the database</p>';
			}
			// Clear the $_POST values, so that the form can be used again
			$_POST=[];
...
<form action="trips.php?page=addadive&tripId=<?php echo $_GET['tripId']; ?>" method="post">
<table class="profile">
...
for ($i = 1; $i <= 31; $i++) {
	echo "<option value=\"$i\"";
	if (isset($_POST['day']) && ($_POST['day'] == $i)) { echo ' selected';}
	echo ">$i</option>";
}?>
</select>
...
foreach ($months as $key => $month) {
	...
	if (isset($_POST['month']) && ($_POST['month'] == ($key+1))) { echo ' selected';}
	echo ">$month</option>";
}?>
</select>
...
for ($i = 0; $i<=23; $i++) {
	if ($i < 10) {
		echo "<option value=\"$i\"";
		if (isset($_POST['hourIn']) && $_POST['hourIn'] == $i) {echo ' selected';}
		echo ">0$i</option>";
	} else {
		echo "<option value=\"$i\"";
		if (isset($_POST['hourIn']) && $_POST['hourIn'] == $i) {echo ' selected';}
...
for ($i = 0; $i<=59; $i++) {
	if ($i < 10) {
		echo "<option value=\"$i\"";
		if (isset($_POST['minutesIn']) && $_POST['minutesIn'] == $i) {echo ' selected';}
		echo ">0$i</option>";
	} else {
		echo "<option value=\"$i\"";
		if (isset($_POST['minutesIn']) && $_POST['minutesIn'] == $i) {echo ' selected';}
...
for ($i = 0; $i<=23; $i++) {
	if ($i < 10) {
		echo "<option value=\"$i\"";
		if (isset($_POST['hourOut']) && $_POST['hourOut'] == $i) {echo ' selected';}
		echo ">0$i</option>";
	} else {
		echo "<option value=\"$i\"";
		if (isset($_POST['hourOut']) && $_POST['hourOut'] == $i) {echo ' selected';}
...
for ($i = 0; $i<=59; $i++) {
	if ($i < 10) {
		echo "<option value=\"$i\"";
		if (isset($_POST['minutesOut']) && $_POST['minutesOut'] == $i) {echo ' selected';}
		echo ">0$i</option>";
	} else {
		echo "<option value=\"$i\"";
		if (isset($_POST['minutesOut']) && $_POST['minutesOut'] == $i) {echo ' selected';}
...
<tr><td>Dive time: </td><td>minutes <input type="number" name="diveTime" size="10" <?php if (isset($_POST['diveTime'])) { echo "value=\"{$_POST['diveTime']}\""; }?>/></td></tr>
<tr><td>Max depth reached: </td><td><select name="units"><option value="mt">meters</option><option value="ft">feet</option>
<input type="number" name="maxDepth" size="10" <?php if (isset($_POST['maxDepth'])) { echo "value=\"{$_POST['maxDepth']}\""; }?>/></td></tr>
<tr><td>Gas: </td><td><input id="air" type="radio" name="gas" value="air" <?php if (!isset($_POST['gas']) || $_POST['gas'] == 'air') {echo ' checked';} ?> onclick="eanCheck();"/>Air
<input type="radio" name="gas" value="ean" <?php if (isset($_POST['gas']) && ($_POST['gas'] == 'ean')) {echo ' checked';}?> onclick="eanCheck();">EAN 
...
// Open the list
echo '<tr><td>Dive Shop: </td><td><select name="diveshop"><option value="" selected>None</option>';
	...
	while($result=$stmt->fetch()) {
		...
		if ($r2) {
			...
			if ($result2=$stmt2->fetch()) {
				...
				if (isset($_POST['diveshop']) && ($_POST['diveshop']==$result['shopId'])) {
					echo ' selected';
				} 
...
foreach ($countries as $country) {
	...
	if (isset($_POST['country']) && $country == $_POST['country']) { echo ' selected';}
	echo ">$country</option>";
}?></select></td></tr>
<tr><td>Dive site name: </td><td><input type="text" name="diveSite" size="30" <?php if (isset($_POST['diveSite'])) { echo "value=\"{$_POST['diveSite']}\"";} ?>/></td></tr>
<tr><td>Marine life spotted: </td><td><input type="text" name="marineLife" size="40" <?php if (isset($_POST['marineLife'])) { echo "value=\"{$_POST['marineLife']}\"";} ?>/></td></tr>
<tr><td>Description: </td><td><textarea name="description" cols="40" rows="5"><?php if (isset($_POST['description'])) { echo $_POST['description'];}?></textarea></td></tr>
