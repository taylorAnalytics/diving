...
if (isset($_GET['galleryId'])) {
	...
	if (isset($_GET['image'])) { // It has, show the image only
		...
		if ($n > 0) { // It's not the first picture
			echo '<p class="left"><a class="button" href="gallery.php?page=view&galleryId=' . $gallery->galleryId . '&image=' . $files[$n-1] . '">Previous picture</a></p>';
		}
		if ($n < count($files) - 1) { // It's not the lasdt piture}
			echo '<p class="right"><a class="button" href="gallery.php?page=view&galleryId=' . $gallery->galleryId . '&image=' . $files[$n+1] . '">Next picture</a></p>';
		}
		echo '<p class="centered"><a class="button" href="gallery.php?page=view&galleryId=' . $gallery->galleryId . '">Back to the gallery</a></p>';
		echo '<div></div>';
		echo '<img class="main" src="./galleries/' . $gallery->galleryId . '/' . $_GET['image'] . '"/>';
	} else { // It hasn't, show the whole gallery
		
	echo '<a class="button-2" href="gallery.php?page=all">Back to all galleries</a>
		<a class="button-2" href="gallery.php?page=edit&galleryId=' . $gallery->galleryId . '">Edit this gallery</a>';
	...
	foreach ($files as $file) {
		echo '<div class="gallery-image"><a href="gallery.php?page=view&galleryId=' . $gallery->galleryId . '&image=' . $file . '">
